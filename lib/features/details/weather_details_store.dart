  ...
  Future<void> requestDetails(
    ...
    try {
      final WeatherDetailsResponse response =
          await _repository.fetchWeatherDetails(lat, lon);
      ...
      info
        ...
        ..add(AdditionalInfo(
            '', 'Wind speed', '${response.current.windSpeed} m/s'))
        ..add(AdditionalInfo(
            '', 'Wind direction', mapDegrees(response.current.windDeg)))
        ..add(
            AdditionalInfo('', 'Feels like', '${response.current.feelsLike}°'))
        ..add(
            AdditionalInfo('', 'Pressure', '${response.current.pressure} hPa'))
        ..add(AdditionalInfo(
            '', 'Visibility', '${response.current.visibility} m'))
